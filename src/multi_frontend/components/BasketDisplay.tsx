...
const BasketDisplay: React.FC = () => {
  ...
  // Format price in MULTI terms
  const formatMultiPrice = (usdPrice: number): string => {
  ...
  const fetchBasketInfo = useCallback(async (isAutoRefresh = false) => {
    try {
      ...
      const canisterId = import.meta.env.VITE_CANISTER_ID_MULTI_BACKEND || 
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-4">
            ...
            {/* Refresh Controls */}
            <div className="flex items-center gap-2 text-sm min-w-[60px]" title={refreshing ? "Fetching latest data..." : "Data is up to date"}>
              <div className={`w-2 h-2 rounded-full transition-colors duration-300 ${
                refreshing ? 'bg-yellow-400' : 'bg-green-400'
              } ${refreshing && autoRefresh ? 'animate-pulse' : ''}`}></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white bg-opacity-10 rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-white">
              {formatAmount(systemInfo.totalSupply)}
            </p>
            <p className="text-xs text-gray-500 mt-1">MULTI tokens</p>
          </div>
          
          <div className="bg-white bg-opacity-10 rounded-lg p-4">
            <p className="text-sm text-gray-400">MULTI Price</p>
            <p className="text-2xl font-bold text-white">
              {formatUSD(multiPrice)}
            </p>
            <p className="text-xs text-gray-500 mt-1">per MULTI token</p>
          </div>
          
          <div className="bg-white bg-opacity-10 rounded-lg p-4">
            <p className="text-sm text-gray-400">Total Value Locked</p>
            <p className="text-2xl font-bold text-white">
              {formatUSD(Number(systemInfo.totalSupply) / 1e8 * multiPrice)}
            </p>
            <p className="text-xs text-gray-500 mt-1">USD equivalent</p>
          </div>
          
          <div className="bg-white bg-opacity-10 rounded-lg p-4">
            <p className="text-sm text-gray-400">Backing Tokens</p>
            <p className="text-2xl font-bold text-white">
              {systemInfo.backingTokens.length}
            </p>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {systemInfo.backingTokens.map((token, index) => {
              ...
              const tokenPrice = priceDisplay === 'usd' 
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p className="text-white font-medium text-lg">{tokenInfo?.name || `Token ${index}`}</p>
